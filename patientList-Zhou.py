...
hpListRaw = requests.get(
    'http://20.21.1.224:5537/api/api/Bed/GetPatientList/%E5%8C%BB%E7%96%97%E7%BB%84/30047/33A/A002', headers=headers).json()
...
for item in hpList:
    if not any(d['mrn'] == item['mrn'] for d in tpList):
        ...
        for d in tpList:
            if d['mrn'] == item['mrn'] and d['name'] != item['pinfo'] and not d['name'].endswith('.'):
                requests.request(
                    "PUT",
