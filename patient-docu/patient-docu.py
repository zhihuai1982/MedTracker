...
# 读取 patient-list.csv 文件的前2列
patient_list = pd.read_csv("patient-list.csv", usecols=[0, 1])

# %%

# 遍历 patient_list 的每一行，打印 ID
for pre_index, pre_row in patient_list.iterrows():
    print(pre_row["Id"], pre_row["PatientLocalMinSn"])
    mrn = int(pre_row["PatientLocalMinSn"])
    ...
    patient_docu = "[TOC] \n\n# " + str(pre_row["Id"]) + patient_series_df.iloc[0,:]["PNAME"] + " " + patient_series_df.iloc[0,:]["MRN"] + "\n"
    ...
    for index, row in patient_series_df.iterrows():
        ...
        if (row["STAY"] == "O" or row["STAY"] == "E")and not any(name in row["DEPTNAME"] for name in ["护理", "门诊办公室", "体检"]):
            ...
            for record in hDocuList:
                ...
                # 如果 docname 包含 "住院证，VTE，七十二，病案首页"，则跳过这次循环
                docnames = ["首次病程录","手术记录","出院记录"]
                if not any(name in record['docname'] for name in docnames):
                    continue
    ...
    with open("./patient_documents/" + str(pre_row["Id"]) + " " + patient_series_df.iloc[0,:]["PNAME"] + " " + patient_series_df.iloc[0,:]["MRN"] + ".md", "w", encoding="utf-8") as f:
