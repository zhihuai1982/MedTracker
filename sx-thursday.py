...
while current_day <= last_day:
    if current_day.weekday() == 3:  # 3代表周四
        thursdays.append(current_day.strftime("%Y-%m-%d"))
...
for thursday_date in thursdays:
    url = (
        f"http://40.22.2.60:5537/api/api/Oper/GetOperArrange/77G/5/A009/{thursday_date}"
    )
    try:
        ...
        if response.status_code == 200:
            ...
            # 为每条数据添加周四日期标识
            for item in data:
                item["thursday_date"] = thursday_date
            all_data.extend(data)
        else:
            print(f"请求失败：{thursday_date}，状态码：{response.status_code}")
    except Exception as e:
        print(f"请求异常：{thursday_date}，错误信息：{str(e)}")
...
for index, row in dong_df.iterrows():
    ...
    summary_entry = {
        ...
        "预计结余": 0,
        "手术费用": surgery_cost,
        "thursday_date": row["thursday_date"],  # 修改为周四日期字段
    }
...
for entry in summary_data:
    weekly_groups[entry["thursday_date"]].append(entry)

# 生成每周四分表
for thursday, entries in weekly_groups.items():
    pContent += f"<br><h2>{thursday} 周汇总表</h2>"
...
# 读取/更新文章ID
try:
    with open("sx-thursday.json", "r") as f:
...
post_data = {
    "title": f"绍兴院区 - Thursday - {datetime.now().strftime('%Y-%m-%d %H:%M')}",
    "status": "publish",
    "content": pContent,
}
...
if response.status_code in [200, 201]:
    with open("sx-thursday.json", "w") as f:
        json.dump(
            {
